...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      fatalln "Local Fabric binary version of $LOCAL_VERSION does not match the versions supported by the dev network."
    fi

    infoln "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match the versions supported by the dev network."
    fi
  done
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Waka-district1 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Waka-district1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Waka-district2 Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Waka-district2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-Maori-orderer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/Waka-district1/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    createWaka-district1

    infoln "Creating Waka-district2 Identities"

    createWaka-district2
  ...
  infoln "Generating CCP files for Waka-district1 and Waka-district2"
...
# call the script to create the channel, join the peers of Waka-district1 and Waka-district2,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to Waka-district1 and Waka-district2, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Waka-district1/msp organizations/fabric-ca/Waka-district1/tls-cert.pem organizations/fabric-ca/Waka-district1/ca-cert.pem organizations/fabric-ca/Waka-district1/IssuerPublicKey organizations/fabric-ca/Waka-district1/IssuerRevocationPublicKey organizations/fabric-ca/Waka-district1/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/Waka-district2/msp organizations/fabric-ca/Waka-district2/tls-cert.pem organizations/fabric-ca/Waka-district2/ca-cert.pem organizations/fabric-ca/Waka-district2/IssuerPublicKey organizations/fabric-ca/Waka-district2/IssuerRevocationPublicKey organizations/fabric-ca/Waka-district2/fabric-ca-server.db'
